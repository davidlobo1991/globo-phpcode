   ...
   <div class="row text-center status">
        <div class="col-md-6 bg-green" style="padding-top: 10px;">
            <p><b>TOTAL:</b> <span id="total"></span> {{ $reservation->getTotalPriceAttribute() }}  €</p>
             <span id="toPay" data-pay="{{ $reservation->getTotalPriceAttribute() }}"></span>
        </div>
        <div class="col-md-6 bg-red" style="padding-top: 10px;">
            <p><b>REMAINING:</b> <span id="remainder"> {{$reservation->remainderToPay()}}</span>€ </p>
        </div>
    </div>
    ...
    <hr>
        ...
        @if($paid == false)
             ...
             <div id="payments" class="form-group col-md-6">
              @if($paid->id == 6)
              {{ Form::hidden('paypal', $global->paypal, ['id' => 'paypal']) }}
               <label class="text-danger">Paypal: <b>{{$global->paypal}} %</b></label>
            ...
            <footer>
                {!! Form::submit(trans('common.save'), ['class' => 'btn btn-success', 'id' => 'sendPayments']) !!}
                 <a class="btn btn-info"
                    href="{{ route($reservation->reservation_type_id.'.show', [$reservation->id])  }}">{{ trans('common.info') }}</a>
